...
class DataEfficientImageTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
                torch.zeros(1, 2 + self.patch_embed.n_patches, embed_dim)
        )
    ...
    def forward(self, x):
        ...
        """
        n_samples = x.shape[0]
        x = self.patch_embed(x) # INPUT: 2 x 3 x 224 x 224 ----> 2 x 196 x 768 [OUTPUT]

        cls_token = self.cls_token.expand(
                n_samples, -1, -1
        )  # (n_samples, 1, embed_dim)  || INPUT --> 1,1,768 -> num_class, 1, 768
        ...
        x = torch.cat((cls_token, dist_token, x), dim=1)  # (n_samples, 1 + n_patches, embed_dim)
